...
def interpolate_path(
    ...
    output lives on a lattice. This helper rounds the lattice corners with an
    open Chaikin pass and then densely samples the rounded path.
    """
    if len(path) <= 1:
        return list(path)
...
def display_path(
    ...
    if len(path) <= 1:
        return list(path)
    if _is_dense_enough(path, dense_time_step):
        return list(path)
